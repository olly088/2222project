...
def run_server():    
    ...
    '''
    run(host=host, port=port, server='gunicorn', keyfile='certs/project.test.key', certfile='certs/project.test.crt', debug=debug)
...
def manage_db():
    ...
    sql_db.database_setup(sql_db.hash_salt_password('admin'))
    sql_db.add_user('Alice', sql_db.hash_salt_password('Ape'), 0)
    sql_db.add_user('Bob', sql_db.hash_salt_password('Boot'), 0)
    sql_db.add_user('Charlie', sql_db.hash_salt_password('Copy'), 0)
    return
